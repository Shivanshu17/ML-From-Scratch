...
class LogisticRegression():
    ...
    def fit(self):
        ...
        if self.optimization == 0:
            sgd_optim = sgd.SGD(params = self.params, data = self.mapped_data, epoch = self.epoch, lr = self.lr, activation = 2, cost = self.cost, huber_point = self.h_p, quantile = self.q )
            self.params = sgd_optim.updated_params
        if self.optimization == 1:
            Adagrad_optim = Adagrad.ADAGRAD(params = self.params, data = self.mapped_data, epoch = self.epoch, lr = self.lr, activation = 2, cost = self.cost, ep = self.ep, huber_point = self.h_p, quantile = self.q)
            self.params = Adagrad_optim.updated_params
        if self.optimization == 2:
            Adadelta_optim = Adadelta.ADADELTA(params = self.params, data = self.mapped_data, epoch = self.epoch, lr = self.lr, activation = 2, cost = self.cost, alpha = self.alpha, ep = self.ep, huber_point = self.h_p, quantile = self.q)
            self.params = Adadelta_optim.updated_params
        if self.optimization == 3:
            Adam_optim = Adam.ADAM(params = self.params, data = self.mapped_data, epoch = self.epoch, lr = self.lr, activation = 2, cost = self.cost, ep = self.ep, b1 = self.b1, b2 = self.b2, huber_point = self.h_p, quantile = self.q)
